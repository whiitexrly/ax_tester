...
class AxeCoreTool(Tool):
        ...
        try:
            ...
            if not axe_path.exists():
                raise FileNotFoundError(
                    f"Axe-core not found at {axe_path}. Run: npm i"
                )
            ...
            raise ToolExecutionError(f"Cannot load axe-core: {str(e)}")
    ...
    def execute(self, url: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            # run axe-core
            axe_results = self._run_async(
                self._run_axe_with_playwright_async(url, timeout, rules)
            )
                ...
                try:
                    ...
                    if 'Timeout' in str(e):
                        raise ToolExecutionError(
                            f"Page load timeout after {timeout}s for {url}"
                        )
                    else:
                        raise ToolExecutionError(f"Navigation error: {str(e)}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2: test_url = "google.com"
    else: test_url = sys.argv[1]
